...
export function addToCart(item: Omit<CartItem, "cantidad">) {
  const existing = _cart.find(i => i.codigo === item.codigo);
  if (existing) {
    existing.cantidad += item.multiplo || 1;
  } else {
    _cart.push({ ...item, cantidad: item.multiplo || 1 });
  }
  window.dispatchEvent(new CustomEvent("cart-updated"));
}

export function getCartCount() {
  return _cart.reduce((s, i) => s + i.cantidad, 0);
}
// ──────────────────────────────────────────────────────────────────
...
export default function CarritoPage() {
  const [items, setItems] = useState<CartItem[]>([..._cart]);

  useEffect(() => {
    const sync = () => setItems([..._cart]);
  ...
  function cambiarCantidad(codigo: number, dir: 1 | -1) {
    _cart = _cart.map(i => {
      ...
      const step  = i.multiplo || 1;
      const nueva = Math.max(step, i.cantidad + dir * step);
  ...
  function eliminar(codigo: number) {
    _cart = _cart.filter(i => i.codigo !== codigo);
    setItems([..._cart]);
    window.dispatchEvent(new CustomEvent("cart-updated"));
  }
